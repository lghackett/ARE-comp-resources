...
I am going to set up a simple environment that just has python 3.8, jupyter notebook, statsmodels, pandas and numpy for some basic metrics. Remember you can always install more packages later, though trying to install the bulk of your packages at the beginning can sometimes help avoid conflicts later. 

Our first step is actually going to be to make sure the conda-forge channel is added to conda. We're not going to worry too much about channels, but they are essentially locations on the internet where certain packages live. To add a channel we are going to use, just type:

```
conda config --add channels conda-forge
```

You don't need to worry too much about channels because Google can tell you what channel to use; I recommend that anytime you want to install a package, just google "install [PACKAGE NAME] with conda" and copy and paste the code they recommend. 
...
Now let's add another package from a different channel to our new environment:
